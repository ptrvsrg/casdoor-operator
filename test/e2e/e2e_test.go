...
import (
	...
	"github.com/ptrvsrg/casdoor-operator/test/utils"
)

// constant parts of the file
const namespace = "casdoor-operator-system"

var _ = Describe(
	"casdoor", Ordered, func() {
		BeforeAll(
			func() {
				// The prometheus and the certmanager are installed in this test
				// because the Casdoor sample has this option enable and
		...
		Context(
			"Casdoor Operator", func() {
				It(
					"should run successfully", func() {
						// var controllerPodName string
						// var err error
						// projectDir, _ := utils.GetProjectDir()
						//
						// // operatorImage stores the name of the image used in the example
						// var operatorImage = "example.com/casdoor-operator:v0.0.1"
						//
						...
						//
						// By("creating an instance of the Casdoor Operand(CR)")
						// EventuallyWithOffset(
						// 	1, func() error {
						// 		cmd = exec.Command(
						// 			"kubectl", "apply", "-f", filepath.Join(
						// 				projectDir,
						// 				"config/samples/cache_v1alpha1_casdoor.yaml",
						// 			), "-n", namespace,
						// 		)
						// 		_, err = utils.Run(cmd)
						// 		return err
						...
						// getCasdoorPodStatus := func() error {
						// 	cmd = exec.Command(
						// 		"kubectl", "get",
						// 		"pods", "-l", "app.kubernetes.io/name=casdoor-operator",
						...
						// 		return fmt.Errorf("casdoor pod in %s status", status)
						// 	}
						// 	return nil
						// }
						// EventuallyWithOffset(1, getCasdoorPodStatus, time.Minute, time.Second).Should(Succeed())
						//
						// By("validating that the status of the custom resource created is updated or not")
						// getStatus := func() error {
						// 	cmd = exec.Command(
						// 		"kubectl", "get", "casdoor",
						// 		"casdoor-sample", "-o", "jsonpath={.status.conditions}",
						// 		"-n", namespace,
						// 	)
						// 	status, err := utils.Run(cmd)
						// 	fmt.Println(string(status))
